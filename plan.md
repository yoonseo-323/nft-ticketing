...
- NFT 발행 트랜잭션을 전송하고 `tx.wait()`로 블록 포함을 확인한 뒤 영수증 이벤트 로그에서 tokenId를 추출한다.
- 발행 완료 확인 즉시 DB를 CONFIRMED로 갱신하고 좌석을 SOLD로 변경한다.
- 데모에서는 PG 결제 없이 구매 요청 즉시 NFT 발행으로 처리한다.
...
- 데모에서는 PG 결제 없이 구매 요청 즉시 NFT 이전으로 처리한다.
...
// Response 200
{
  ...
  "qrData": "0:uuid:0:1748000000:0x1a2b3c..."
}
// qrData를 react-qr-code에 그대로 넘겨서 QR 렌더링
// signature는 서버 이더리움 개인키로 ECDSA 서명한 0x 시작 132자 hex 문자열
// 60초마다 재요청하여 QR 갱신
```
